...
function HomeGuest() {
    ...
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value && !/^([a-zA-Z0-9]+)$/.test(draft.username.value)) {
          draft.username.hasErrors = true;
          draft.username.message = "Username can only contains letters and numbers";
        }
        return;
      ...
      case "emailAfterDelay":
        if (!/^\S+@\S+\.\S+$/.test(draft.email.value)) {
          ...
          draft.email.message = "Please provide a valid e-mail address";
        }
      ...
      case "emailUniqueResults":
        if (action.value) {
          ...
          draft.email.message = "That e-mail is already being in use.";
        } else {
      ...
      case "passwordImmediately":
        ...
        if (draft.password.value.length > 50) {
          ...
          draft.password.message = "The password cannot exceed 50 characters";
        }
        return;
      case "passwordAfterDelay":
        if (draft.password.value.length < 12) {
          draft.password.hasErrors = true;
          draft.password.message = "The password must contains at least 12 characters";
        }
        return;
      case "submitForm":
        draft.submitCount++;
  ...
  function isEverythingGood() {
    if (!state.username.hasErrors && state.username.isUnique && !state.email.hasErrors && state.email.isUnique && !state.password.hasErrors) return true;
