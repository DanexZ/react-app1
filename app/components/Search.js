    ...
    if (state.requestCount) {
      ...
      async function fetchResults() {
        try {
          ...
          setState((draft) => {
            draft.results = response.data.posts;
            draft.show = "results";
          });
        } catch (e) {
          console.log(e);
      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results" + (state.show == "results" ? " live-search-results--visible" : "")}>
            {Boolean(state.results.length) && (
              <div className="list-group shadow-sm">
                <div className="list-group-item active">
                  <strong>Search Results</strong> ({state.results.length} {state.results.length > 1 || state.results.length == 0 ? "items" : "item"} found)
                </div>
            ...
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-sm">Sorry, we could not find any results for thath search.</p>}
          </div>
        </div>
      </div>
    </>
